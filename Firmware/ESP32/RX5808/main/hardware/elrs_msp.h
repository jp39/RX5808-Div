...
#define MSP_V2_FLAG_REQUEST     '<'
#define MSP_V2_FLAG_RESPONSE    '>'
#define MSP_V2_FLAG_ERROR       '!'
...
typedef enum {
    ...
    MSP_HEADER_X,
    MSP_HEADER_V2_FLAGS,
    MSP_HEADER_V2_FUNC_L,
    MSP_HEADER_V2_FUNC_H,
    MSP_HEADER_V2_SIZE_L,
    MSP_HEADER_V2_SIZE_H,
    MSP_PAYLOAD_V2,
    MSP_CHECKSUM_V2
} msp_state_t;
...
static inline uint8_t crc8_dvb_s2(uint8_t crc, uint8_t ch) {
    return crc8_dvb_s2_table[crc ^ ch];
}

// Calculate CRC8 for a buffer
static inline uint8_t crc8_dvb_s2_buf(const uint8_t *buf, uint16_t len) {
    uint8_t crc = 0;
...
static inline void msp_parser_init(msp_parser_t *parser) {
